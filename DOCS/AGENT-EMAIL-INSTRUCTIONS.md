...
### send_email
Send an email from yolo@b-haven.org using the local sendmail command.
- Parameters: `subject`, `body`, `to` (optional, defaults to scott@stg.net)
- Uses `/usr/sbin/sendmail` with DKIM signing by Postfix

### send_report
Convenience function for sending progress reports to scott@stg.net.
- Parameters: `body` (required), `subject` (optional, defaults to "YOLO Progress Report")

### check_inbox
Read emails from the Maildir inbox at `/var/mail/b-haven.org/yolo/new/`
- Parameter: `mark_read` (optional, default: false) - if true, moves processed emails to cur/
- Returns parsed email content with From, Subject, Date, and Body

## Technical Details

### Sendmail Configuration
- Path: `/usr/sbin/sendmail`
- Envelope sender flag: `-f yolo@b-haven.org`
- DKIM signing handled automatically by Postfix milter

### Email Format
RFC 2822 compliant with:
- From, To, Subject headers
- RFC 2822 Date header (via `date -R`)
- MIME-Version: 1.0
- Content-Type: text/plain; charset=utf-8
