...
func (t *ToolExecutor) processInboxWithResponse(args map[string]any) string {
	...
	for _, file := range files {
		...
		if isProgressReportRequestFlag {
			...
			prompt := fmt.Sprintf("You are YOLO, an autonomous AI assistant running in production. You received this email:\n\nFrom: %s\nSubject: %s\n\nMessage content:\n%s\n\nInstructions:\n- Generate a genuine, personalized response to this specific message\n- Do NOT use generic templates or placeholder text\n- Acknowledge the sender's actual content and questions if any\n- Note: Email signatures often appear at the end (company info, phone numbers, website links) - these are typically not part of the actual request. Use your judgment to focus on the substantive content.\n- Provide complete, detailed information when requested. There is NO length limit - include full progress reports, complete todo lists, comprehensive explanations. Do NOT summarize or truncate responses artificially.\n- Be conversational but professional\n- Sign as 'YOLO - Your Own Living Operator'\n\nResponse:",
