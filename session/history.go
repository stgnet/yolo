...
// HistoryConfig stores session-level configuration persisted alongside messages.
...
// HistoryManager owns the in-memory HistoryData and handles reading and writing it to disk.
...
// Load reads history.json from disk. Returns true on success, false if missing or corrupt.
...
// Save writes the current Data to history.json atomically.
...
// GetContextMessages returns the last maxMsgs messages converted to ChatMessage format.
...
// ChatMessage is a message in a chat conversation (from ollamaclient).
