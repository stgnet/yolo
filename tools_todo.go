...
func (te *ToolExecutor) addTodo(args map[string]any) string {
	...
	if !ok || strings.TrimSpace(title) == "" {
		return errorMessage("title parameter is required and cannot be empty")
	}

	item, err := GetGlobalTodoList().Add(title)
	if err != nil {
...
func (te *ToolExecutor) completeTodo(args map[string]any) string {
	...
	found, err := GetGlobalTodoList().Complete(title)
	if err != nil {
		return errorMessage("%v", err)
	}
	if found {
	...
	return errorMessage("TODO not found or already completed: %s", title)
}
...
func (te *ToolExecutor) deleteTodo(args map[string]any) string {
	...
	found, err := GetGlobalTodoList().Delete(title)
	if err != nil {
		return errorMessage("%v", err)
	}
	if found {
	...
	return errorMessage("TODO not found: %s", title)
}
